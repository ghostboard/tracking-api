...
describe('getSlug', () => {
  ...
  test('get a slug keeping query params', () => {
    ...
    const slug =
      '/about-me?utm_content=buffer197fa&utm_medium=social&utm_source=twitter.com&utm_campaign=buffer';
    const path = `https://davidburgos.blog/about-me/?utm_content=buffer197fa&utm_medium=social&utm_source=twitter.com&utm_campaign=buffer`;
    expect(getSlug(blog, path)).toEqual(slug);
  });
  ...
  test('issue getting wrong slug with www with query', () => {
    ...
    const slug =
      '/senior-finance-executive---farmiso-logistics-ap?id=16312c3d-555a-4171-89bd-c937f827483f';
  ...
  test('simple slug option', () => {
    ...
    const slug =
      '/jobs/senior-finance-executive---farmiso-logistics-ap/amp?id=16312c3d-555a-4171-89bd-c937f827483f';
    const path = `https://www.meesho.io/jobs/senior-finance-executive---farmiso-logistics-ap/amp?id=16312c3d-555a-4171-89bd-c937f827483f`;
    expect(getSlug(blog, path, false, true)).toEqual(slug);
  });

  test('simple slug option without query', () => {
    const blog = {
      domain: 'meesho.io',
      url: 'https://meesho.io/',
    };
    const slug = '/jobs/senior-finance-executive---farmiso-logistics-ap/amp';
    const path = `https://www.meesho.io/jobs/senior-finance-executive---farmiso-logistics-ap/amp?id=16312c3d-555a-4171-89bd-c937f827483f`;
    expect(getSlug(blog, path, true, true)).toEqual(slug);
  });
});
