...
export default async function saveView(
  params: any,
  req: FastifyRequest
): Promise<string[]> {
  ...
  if (url.query) {
    ...
    if (UTMCampaign) {
      newVisit.UTMCampaign = Array.isArray(UTMCampaign)
        ? UTMCampaign[0]
        : UTMCampaign;
    }
    if (UTMTerm) {
    ...
    if (UTMContent) {
      newVisit.UTMContent = Array.isArray(UTMContent)
        ? UTMContent[0]
        : UTMContent;
    }
  }
  ...
  const isUTMEmail =
    newVisit.UTMMedium &&
    newVisit.UTMMedium.toLowerCase &&
    newVisit.UTMMedium.toLowerCase() === 'email';
