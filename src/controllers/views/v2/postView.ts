...
export async function handler(
  ...
  try {
    ...
    if (!blog) {
      return res
        .code(404)
        .send({ message: `Blog not found with ID=${blogId}` });
    }
    ...
    const dontMatchNewDomain =
      blog.newDomain && referer && !referer.includes(blog.newDomain);
    ...
    if (blockIP) {
      console.log(
        `>> block IP ${userIP} ${ipFilters} isPrivate=${isPrivateIP}`
      );
      ...
      console.log(
        `>> req.headers['X-Real-IP'] = ${req.headers['x-real-ip']} / ${req.headers['X-Real-IP']}`
      );
    ...
    const isInternalReferer =
      hasReferer &&
      visit.referer.includes(viewParams.blog.domain || viewParams.blog.url);
    ...
    const todoAfterPost = [afterView(viewParams, req), emitDashboard(blogId)];
